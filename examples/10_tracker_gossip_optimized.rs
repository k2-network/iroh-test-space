...
async fn main() -> Result<()> {
    ...
    let gossip_topic = if peer_node_ids.is_empty() {
        ...
        // OPTIMIZATION: We only need to connect to ONE peer to join the swarm successfully.
        // We shuffle the list (or take first N) to avoid thundering herd if possible, but for now just take the list.
        
        // Randomize list to distribute load (optional but recommended)
        ...
        system!("Attempting to connect to {} peers (max 10)...", targets.len());

        let join_fut = gossip.subscribe_and_join(topic_id, targets);
        
        // Timeout expanded to 10s as requested
        match tokio::time::timeout(Duration::from_secs(10), join_fut).await {
            Ok(res) => match res {
                Ok(topic) => {
                    system!("Successfully joined gossip swarm!");
                    topic
                },
                Err(e) => {
                    error!("Initial join failed: {}. Subscribing alone.", e);
                    gossip.subscribe(topic_id, vec![]).await?
                }
            },
            Err(_) => {
                system!("Connection timeout (10s). Subscribing alone to receive future messages.");
                gossip.subscribe(topic_id, vec![]).await?
            }
        }
    };
