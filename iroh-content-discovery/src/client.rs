...
pub enum Error {
    ...
    #[snafu(display("Failed to complete handshake: {}", source))]
    Handshake {
        source: iroh::endpoint::AuthenticationError,
        backtrace: snafu::Backtrace,
    },
}
...
pub async fn announce(
    ...
    // iroh 0.95: into_0rtt returns Result<OutgoingZeroRttConnection, Connecting>
    // OutgoingZeroRttConnection has open_bi() directly and can be awaited for full handshake
    match connecting.into_0rtt() {
        Ok(zero_rtt_conn) => {
            trace!("connected to tracker using possibly 0-rtt: {endpoint_id}");
            // Use open_bi directly on zero_rtt_conn
            let (mut send, recv) = zero_rtt_conn.open_bi().await.context(OpenStreamSnafu)?;
            let request = Request::Announce(signed_announce);
            let request_bytes = postcard::to_stdvec(&request).context(SerializeRequestSnafu)?;
            trace!("sending announce");
            send.write_all(&request_bytes).await.context(WriteRequestSnafu)?;
            ...
            let _status = zero_rtt_conn.handshake_completed().await.context(HandshakeSnafu)?;
            Ok(())
        }
        Err(connecting) => {
...
pub async fn query_conn(
    ...
    let request = Request::Query(args);
    let request = postcard::to_stdvec(&request).context(SerializeRequestSnafu)?;
    // iroh 0.95: remote_id() returns PublicKey directly, not Result
    trace!(
        "connected to {:?}",
